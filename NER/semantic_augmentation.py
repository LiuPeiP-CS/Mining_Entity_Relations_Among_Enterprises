...
"""
class SemanticAug(nn.Module): # 首次运行时，使用该类进行计算；首次运行成功后，使用后面的同名类直接导入数据
    def __init__(self, vector_file, word_alphabet, semantic_emb_dim, file_addr):  # 打开预训练好的词向量文件
        super(SemanticAug, self).__init__()
        self.word_alphabet = word_alphabet
        self.semantic_emb_dim = semantic_emb_dim
        self.sem_aug_addr = file_addr
    ...
    def semantic_emb_talbe(self):
        ...
        np.save(self.sem_aug_addr, emb_table)
        print('************The SemanAug Representation Is Saved!***********')
        return emb_table
...
class SemanticAug(nn.Module):
    ...
    def semantic_emb_talbe(self):
        return np.load(self.sem_aug_addr)
